...
func initHandlers() {
	...
	http.Handle(fmt.Sprintf("%s/checktrips", root), httptransport.NewServer(
		transport.TripsCheckEndpoint(svc), transport.DecodeCheckTripRequest, transport.EncodeResponse,
	))
