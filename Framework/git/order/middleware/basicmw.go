...
func (mw BasicMiddlewareStruct) CheckHandler(ctx context.Context, request cm.MyTrips) cm.MytripsResponse {
	
	defer func(begin time.Time) {
		log.WithField("execTime", float64(time.Since(begin).Nanoseconds())/float64(1e6)).Info("CheckHandler ends")
	}(time.Now())

	log.WithField("request", request).Info("CheckHandler begins")

	return mw.PaymentServices.CheckHandler(ctx, request)
